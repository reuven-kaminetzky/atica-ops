// Shopify API version — configurable via env var
// Shopify drops versions ~12 months after release
// Check: curl https://STORE.myshopify.com/admin/api/VERSION/shop.json -H "X-Shopify-Access-Token: TOKEN"
...
class ShopifyClient {
  constructor({ shop, accessToken }) {
    ...
    this.version = SHOPIFY_API_VERSION;
...
async function createClient() {
  ...
  for (const shop of candidates) {
    try {
      ...
      _clientExpiry = Date.now() + 5 * 60 * 1000;
      _resolvedShop = shop;
      if (shop !== process.env.SHOPIFY_STORE_URL) {
        console.log(`[shopify] Connected to ${shop} (env var says ${process.env.SHOPIFY_STORE_URL || 'unset'})`);
      }
      return client;
    } catch (err) {
      console.warn(`[shopify] ${shop} failed: ${err.message}`);
    }
  }

  console.error('[shopify] All store URLs failed. Tried:', candidates.join(', '));
  return null;
}
