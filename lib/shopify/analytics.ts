// ═══════════════════════════════════════════════════════════════
// Stallon: Order analytics — velocity, sales rollups
// ═══════════════════════════════════════════════════════════════

import type { ShopifyOrder, VelocityEntry, SalesSummary, DailySales } from './types';

export function sinceDate(days: number): string {
  return new Date(Date.now() - days * 86400000).toISOString();
}

export function buildVelocity(orders: ShopifyOrder[], days: number): VelocityEntry[] {
  const bySku: Record<string, { sku: string; title: string; units: number; revenue: number }> = {};
...
export function buildSalesSummary(orders: ShopifyOrder[], days: number): SalesSummary {
  let totalRevenue = 0;
  let totalUnits = 0;
  const byDay: Record<string, DailySales> = {};
