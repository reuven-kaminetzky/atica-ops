 ...
 * delivery, store compliance. The bridge between Supply Chain and Inventory.
 */
...
const transfer = {
  ...
  async advanceStatus(id, { status, by }) {
    ...
    // Explicit branches — no dynamic column names, no SQL injection
    let updated;
    switch (status) {
      case 'picked':
...
const van = {
  ...
  async depart(id) {
    const db = sql();
    const [updated] = await db`
      UPDATE van_routes SET status = 'departed', departed_at = NOW()
      WHERE id = ${id} RETURNING *
    `;
    return updated;
  },
  ...
  async complete(id) {
    const db = sql();
    const [updated] = await db`
      UPDATE van_routes SET status = 'completed', completed_at = NOW()
      WHERE id = ${id} RETURNING *
    `;
    return updated;
  },
