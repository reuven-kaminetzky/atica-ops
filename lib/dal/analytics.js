...
const analytics = {
  ...
  async getBreakdown(opts = {}) {
    ...
    const columns = (opts.columns || ['stock', 'sales', 'velocity']).filter(c => VALID_COLUMNS.includes(c));
    const sortCol = VALID_COLUMNS.includes(opts.sort?.column) ? opts.sort.column : 'stock';
    ...
    const rows = await db.unsafe(`
      WITH sales_by_mp AS (
        ...
        ${filters.location ? `AND store = '${filters.location.replace(/'/g, "''")}'` : ''}
        GROUP BY mp_id
      ),
      po_by_mp AS (
        SELECT mp_id,
      ...
      WHERE mp.external_ids IS NOT NULL
        AND array_length(mp.external_ids, 1) > 0
        ${filters.category ? `AND mp.category = '${filters.category.replace(/'/g, "''")}'` : ''}
        ${filters.vendor ? `AND mp.vendor_id = '${filters.vendor.replace(/'/g, "''")}'` : ''}
        ${filters.mp ? `AND mp.id = '${filters.mp.replace(/'/g, "''")}'` : ''}
        ${filters.outOfStock ? 'AND mp.total_inventory = 0' : ''}
        ${filters.signal ? `AND mp.signal = '${filters.signal.replace(/'/g, "''")}'` : ''}
      GROUP BY ${dim.select}, ${dim.label}
      ORDER BY ${sortCol} ${sortDir} NULLS LAST
      LIMIT ${parseInt(filters.limit) || 200}
    `);
