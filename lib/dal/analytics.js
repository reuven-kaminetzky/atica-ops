...
const DIMENSIONS = {
  category: {
    ...
    label: 'mp.category',
    join: '',
    groupBy: 'mp.category, mp.category',
  },
  vendor: {
    select: 'mp.vendor_id',
    label: "COALESCE(v.name, mp.vendor_id, 'No Vendor')",
    join: 'LEFT JOIN vendors v ON v.id = mp.vendor_id',
    groupBy: "mp.vendor_id, COALESCE(v.name, mp.vendor_id, 'No Vendor')",
  },
  mp: {
    select: 'mp.id',
    label: 'mp.name',
  ...
  fit: {
    ...
    join: 'LEFT JOIN styles st ON st.mp_id = mp.id LEFT JOIN skus sk ON sk.style_id = st.id',
    groupBy: "sk.fit, COALESCE(sk.fit, 'One Fit')",
  },
  size: {
    select: 'sk.size',
    label: 'sk.size',
    join: 'LEFT JOIN styles st ON st.mp_id = mp.id LEFT JOIN skus sk ON sk.style_id = st.id',
    groupBy: 'sk.size, sk.size',
  },
  length: {
    select: 'sk.length',
    label: "COALESCE(sk.length, 'Standard')",
    join: 'LEFT JOIN styles st ON st.mp_id = mp.id LEFT JOIN skus sk ON sk.style_id = st.id',
    groupBy: "sk.length, COALESCE(sk.length, 'Standard')",
  },
  location: {
    select: 'ie.location_code',
    label: 'ie.location_code',
    join: 'LEFT JOIN styles st ON st.mp_id = mp.id LEFT JOIN skus sk ON sk.style_id = st.id LEFT JOIN inventory_events ie ON ie.sku_id = sk.id',
    groupBy: 'ie.location_code, ie.location_code',
  },
  grade: {
    select: 'st.grade',
    label: 'st.grade',
    join: 'LEFT JOIN styles st ON st.mp_id = mp.id',
    groupBy: 'st.grade, st.grade',
  },
};
...
const SORT_COLUMNS = {
  ...
  velocity: 'velocity', x_rate: 'x_rate', days: 'days_of_stock',
  incoming: 'incoming', style_count: 'style_count', sku_count: 'sku_count',
  mp_count: 'mp_count',
};
...
const analytics = {
  ...
  async getBreakdown(opts = {}) {
    ...
    // No hard filter on external_ids — show all MPs, even unsynced ones.
    // Unsynced MPs will have 0 stock/velocity, which is correct.
    ...
    const query = `
      ...
      SELECT
        ...
        COALESCE(MIN(NULLIF(mp.days_of_stock, 999)), 0)::int AS days_of_stock,
      ...
      WHERE ${whereSQL}
      GROUP BY ${dim.groupBy || `${dim.select}, ${dim.label}`}
    ...
    let rows = [];
    try {
      // Use db.query() — db.unsafe() ignores params in @neondatabase/serverless
    ...
    return {
      ...
      rows: rows.map(r => ({
        key: r.group_key,
        label: r.group_label,
        mpId: groupBy === 'mp' ? r.group_key : undefined,
        mp_count: r.mp_count,
        stock: r.stock,
        sales: r.sales,
        revenue: parseFloat(r.revenue || 0),
        velocity: parseFloat(r.velocity || 0),
        x_rate: parseFloat(r.x_rate || 0),
        days: parseInt(r.days_of_stock || 0),
        incoming: r.incoming,
