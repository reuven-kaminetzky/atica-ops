...
const purchaseOrders = {
  ...
  async create(data) {
    ...
    if (fobTotal > 0) {
      const { PAYMENT_TERMS } = require('../constants');

      // Resolve terms: explicit > vendor preference > standard
      let terms = data.paymentTerms;
      if (!terms || terms === 'standard') {
        // Check if vendor has preferred terms
        if (data.vendorId) {
          const [vendor] = await db`SELECT preferred_terms FROM vendors WHERE id = ${data.vendorId}`.catch(() => [null]);
          if (vendor?.preferred_terms) terms = vendor.preferred_terms;
        }
      }
      if (!terms) terms = 'standard';

      const splits = PAYMENT_TERMS[terms] || PAYMENT_TERMS.standard;
  ...
  async advanceStage(id, { checkedBy = null } = {}) {
    ...
    // Atomic advance — WHERE stage = current prevents race conditions.
    // If another request advanced between our SELECT and UPDATE, this returns 0 rows.
    const result = await db`
      UPDATE purchase_orders SET stage = ${nextStage.name}, stage_index = ${nextStage.id}
      WHERE id = ${id} AND stage = ${po.stage}
      RETURNING *
    `;

    if (result.length === 0) {
      return { error: 'Conflict: PO stage was changed by another request. Reload and try again.' };
    }
  ...
  async update(id, fields) {
    ...
    `;
    if (updated) audit('po', id, 'updated', fields);
    return updated;
  },
