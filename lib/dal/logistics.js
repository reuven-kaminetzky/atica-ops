 ...
 * 
 * Tables: receiving_log, transfers, van_routes
...
const receiving = {
  ...
  async complete(id, receivedItems, discrepancies) {
    const db = sql();
    const status = discrepancies?.length > 0 ? 'disputed' : 'complete';
    const [updated] = await db`
      ...
      WHERE id = ${id} RETURNING *
    `;
    if (updated) audit('receiving', id, status, { items: receivedItems?.length, discrepancies: discrepancies?.length });
    return updated;
  },
...
const transfers = {
  ...
  async create({ fromLocation, toLocation, items, createdBy }) {
    ...
    const [created] = await db`
      ...
      RETURNING *
    `;
    audit('transfer', id, 'created', { from: fromLocation, to: toLocation, units: totalUnits }, createdBy);
    return created;
  },
  ...
  async advanceStatus(id, { status, by }) {
    ...
    if (updated) audit('transfer', id, status, { by });
    return updated;
  },
...
module.exports = { receiving, transfers, vanRoutes, getDashboard };
