...
/**
 * Get the PLM lifecycle stages.
 * Every MP moves through these stages from concept to retirement.
 */
const PLM_STAGES = [
  { id: 1,  name: 'Concept',           gate: null },
  { id: 2,  name: 'Design Brief',      gate: 'PD review' },
  { id: 3,  name: 'Sampling',          gate: null },
  { id: 4,  name: 'Sample Review',     gate: 'PD approval' },
  { id: 5,  name: 'Costing',           gate: 'Finance review' },
  { id: 6,  name: 'Cost Approved',     gate: 'Finance approval' },
  { id: 7,  name: 'Pre-Production',    gate: null },
  { id: 8,  name: 'PO Created',        gate: null },
  { id: 9,  name: 'Production',        gate: null },
  { id: 10, name: 'QC',                gate: 'PD inspection' },
  { id: 11, name: 'Shipped',           gate: null },
  { id: 12, name: 'In Transit',        gate: null },
  { id: 13, name: 'Customs',           gate: null },
  { id: 14, name: 'Warehouse',         gate: null },
  { id: 15, name: 'Distribution',      gate: null },
  { id: 16, name: 'In-Store',          gate: null },
  { id: 17, name: 'Reorder Review',    gate: 'PD + Finance' },
  { id: 18, name: 'End of Life',       gate: null },
];

// ── Exports ─────────────────────────────────────────────────

// ── Seasonal Multipliers ────────────────────────────────────
// Adjust velocity forecasts by season. From original prototype.
// Month → multiplier. Used in production planning and cash flow projections.

const SEASONAL_MULTIPLIERS = {
  1: 0.85, 2: 0.85, 3: 0.85,   // Spring
  4: 0.85, 5: 0.85, 6: 0.85,   // Summer
  7: 1.00, 8: 1.40, 9: 1.40,   // Back-to-School
  10: 1.15, 11: 1.60, 12: 1.60, // Fall/Holiday
};
...
function classifyDemand(sellThrough, velocityPerWeek) {
  if (sellThrough >= 85 && velocityPerWeek >= 5) return 'hot';
  if (sellThrough >= 70 && velocityPerWeek >= 3) return 'rising';
  if (sellThrough < 40 && velocityPerWeek < 2)   return 'slow';
  return 'steady';
}

// ── Distribution Weights ────────────────────────────────────
// Store allocation targets for incoming PO stock.
...
// ── Landed Cost ─────────────────────────────────────────────
// Per-product duty from seeds, plus freight estimate.

function landedCost(fob, dutyPct, freightPct = 8) {
  // duty from product seed, freight ~8% default
